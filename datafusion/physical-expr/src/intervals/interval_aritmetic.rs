...
impl Display for IntervalBound {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "IntervalBound [{}], open={}", self.value, self.open)
    }
}
...
impl Interval {
    ...
    /// Check if the interval `other` is a strict subset of this interval.
    pub(crate) fn is_strict_subset<T: Borrow<Interval>>(&self, other: T) -> Result<bool> {
        let rhs = other.borrow();

        // If the intervals do not intersect at all, return early.
        if (!self.lower.is_unbounded()
            && !rhs.upper.is_unbounded()
            && self.lower.value > rhs.upper.value)
            || (!self.upper.is_unbounded()
        ...
        let lower_shrunk = match (&self.lower.is_unbounded(), &rhs.lower.is_unbounded()) {
            (true, true) => false,
            (true, false) => true,
            (false, true) => {
                return Ok(false);
            ...
            (false, false) => {
                // Both low-side endpoints are finite, compare them. When doing
                // so, take openness into account.
                match (self.lower.open, rhs.lower.open) {
                    (true, true) | (false, false) => self.lower.value < rhs.lower.value,
                    (false, true) => {
                        self.lower.value < next_value::<true>(rhs.lower.value.clone())
                    }
                    (true, false) => {
                        next_value::<true>(self.lower.value.clone()) < rhs.lower.value
                    }
                }
            }
        };

        let upper_shrunk = match (&self.upper.is_unbounded(), &rhs.upper.is_unbounded()) {
            (true, true) => false,
            (true, false) => true,
            (false, true) => {
                return Ok(false);
            }
            (false, false) => match (self.upper.open, rhs.upper.open) {
                // Both low-side endpoints are finite, compare them. When doing
                // so, take openness into account.
                (true, true) | (false, false) => self.upper.value > rhs.upper.value,
                (false, true) => {
                    self.upper.value > next_value::<false>(rhs.upper.value.clone())
                }
                (true, false) => {
                    next_value::<false>(self.upper.value.clone()) > rhs.upper.value
                }
            },
        };

        Ok(lower_shrunk || upper_shrunk)
    }
...
fn next_value<const INC: bool>(value: ScalarValue) -> ScalarValue {
    ...
    match value {
        ...
        Boolean(Some(_)) => Boolean(Some(INC)),
        Int8(Some(val)) => {
...
mod tests {
    ...
    #[test]
    fn strict_subset_tests() -> Result<()> {
        assert!(!open_open::<i32>(Some(24), None)
            .is_strict_subset(open_open::<i32>(Some(24), None))?);
        assert!(open_open::<i32>(Some(24), None)
            .is_strict_subset(open_open::<i32>(Some(25), None))?);
        assert!(open_open::<i32>(None, None)
            .is_strict_subset(open_open::<i32>(Some(25), None))?);
        assert!(!open_open::<i32>(None, None)
            .is_strict_subset(open_open::<i32>(None, None))?);
        assert!(!open_open::<i32>(None, None)
            .is_strict_subset(open_open::<i32>(None, None))?);
        assert!(!open_open::<i32>(Some(24), None)
            .is_strict_subset(open_open::<i32>(None, None))?);
        assert!(!open_open::<i32>(None, Some(24))
            .is_strict_subset(open_open::<i32>(None, None))?);
        assert!(!closed_closed(None, Some(false))
            .is_strict_subset(closed_closed(None, Some(true)))?);
        assert!(closed_closed(None, Some(true))
            .is_strict_subset(closed_closed(None, Some(false)))?);
        assert!(!closed_closed(Some(false), Some(false))
            .is_strict_subset(closed_closed(Some(false), Some(true)))?);
        assert!(closed_closed(Some(false), Some(true))
            .is_strict_subset(closed_closed(Some(false), Some(false)))?);
        Ok(())
    }
